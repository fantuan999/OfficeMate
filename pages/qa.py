...
# ── 处理推荐问题的 URL 参数 ────────────────────────────────
# 推荐问题点击后通过 ?q=... 传入，这里提前取出避免 session 初始化覆盖
_url_question = st.query_params.get("q", "")
if _url_question:
...
# 将 URL 里取到的问题推入 pending
if _url_question:
    st.session_state.pending_question = _url_question
...
def _process_question(prompt: str):
    ...
    with st.chat_message("assistant"):
        ...
        answer, sources, q_type = result["answer"], result["sources"], result["question_type"]
        st.markdown(answer)
        if sources:
            with st.expander(f"引用来源（{len(sources)} 个文档）"):
                for src in sources:
